...
// * sass: sass partials that make up bootstrap-sass styleshee
{
  ...
  function copyFiles(source, dest) {
    return gulp
      .src(path.join(__dirname, "node_modules/bootstrap-sass/assets", source))
      .pipe(gulp.dest(dest));
  }
}
...
// sass
{
  ...
  var includePaths = [__dirname, path.join(__dirname, "scss")];
  ...
  gulp.task("sass:bootstrap", function buildBootstrapCSS() {
    return gulp
      ...
      .pipe(
        transformSelectors(removeOutsideNamespaceMarker, {
          splitOnCommas: true,
        })
      )
      .pipe(gulp.dest("css/"));
  ...
  gulp.task("sass:civicrm", function buildCiviCRMCSS() {
    return gulp
      .src("scss/civicrm/custom-civicrm.scss")
      .pipe(
        bulk({
          includePaths: includePaths,
        })
...
// release
{
  ...
  gulp.task("release:update-info", () => {
    return gulp
      ...
      .pipe(
        replace(
          /<version>([^>]+)<\/version>/g,
          `<version>${argv.ver}</version>`
        )
      )
      .pipe(
        replace(
          /<releaseDate>([^>]+)<\/releaseDate>/g,
          `<releaseDate>${formatTodayDate()}</releaseDate>`
        )
      )
      .pipe(gulp.dest("."));
  });
  ...
  gulp.task("release:commit-changes", () => {
    return git.add(["info.xml", "*.css"]).then(() => {
  ...
  gulp.task("release:params-check", (done) => {
    if (typeof argv.ver === "undefined") {
      throw new PluginError("release:params-check", {
        message:
          'Please specify the version using --ver (example: "--ver 1.1.0")',
  ...
  gulp.task(
    ...
    gulp.series(
      ...
      function releasePrepared(done) {
        ...
        console.log(
          `Branch ${releaseBranchName()} prepared and pushed upstream`
        );
